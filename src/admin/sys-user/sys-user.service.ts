...
export class SysUserService {
  ...
  public async create(createSysUserDto: CreateSysUserDto): Promise<SysUser> {
    // Query whether the username exists
    const isExist = await this.sysUser.exist({ where: { username: createSysUserDto.username } })
    if (isExist)
      throw new ConflictException('Username already exists')

    const user = this.sysUser.create(createSysUserDto)
